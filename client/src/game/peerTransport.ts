...
type Message = { type: "state"; state: PublicGameState } | { type: "action"; action: ClientAction } | { type: "hello"; name: string } | { type: "welcome"; state: PublicGameState };
...
export class FreePeerTransport {
  ...
  async join(hostPeerId: string, roomCode: string, name: string) {
    this.guestName = name || "PLAYER 2"; this.slot = "player2"; this.roomCode = roomCode; this.hostPeerId = hostPeerId; this.peer = new Peer();
